...
export const Q=12;
export const KEYS={'C major':{tonic:28,acc:{},chord:[60,64,67]},'G major':{tonic:32,acc:{3:1},chord:[67,71,74]},'F major':{tonic:31,acc:{6:-1},chord:[65,69,72]},'A minor':{tonic:33,acc:{},chord:[69,72,76]}};
export const METERS=['2/4','3/4','4/4','6/8'];
export const barTicks=s=>{const [n,d]=(s.meter||'4/4').split('/').map(Number);return n*48/d;};
export const beatTicks=s=>s.meter==='6/8'?18:12;
export const beatCount=s=>barTicks(s)/beatTicks(s);
...
export function generate(seed,level=0,bpm=80,bars=1,options={}){
 const meter=options.meter||'4/4',key=options.key||'C major';if(!METERS.includes(meter)||!KEYS[key])throw Error('Choose a supported key and meter.');const settings={meter,key},span=barTicks(settings);
 ...
 if(meter==='6/8'){bank=[[18],[36]];if(level>=3)bank.push([6,6,6],[12,6],[6,12]);if(level>=6)bank.push([3,3,6,6],[6,3,3,6]);}
 const pitches=level===0?[28]:level===1?[28,29]:level<=3?[28,29,30]:[28,29,30,31,32];let step=28,events=[];
 for(let b=0;b<bars;b++){let left=span;while(left){const cell=pick(bank.filter(c=>c.reduce((s,x)=>s+x,0)<=left));for(const duration of cell){if(level)step=pick(pitches.filter(s=>Math.abs(s-step)<=(level>=4?2:1)));const rest=events.length>0&&rng()<.18;events.push({duration,step:rest?null:step,acc:0,rest,tie:false});left-=duration;}}}
 const shift=KEYS[key].tonic-28;events=events.map(e=>e.rest?e:{...e,step:e.step+shift,acc:keyAcc(settings,e.step+shift)});
 return {seed,level,bpm,bars,meter,key,version:VERSION,events:reflow(events)};
}
...
export const comparable=a=>JSON.stringify([a.target.level,a.target.bpm,a.target.bars,a.policy.mode,a.policy.support,a.policy.hearings,a.policy.rhythmOnly,a.target.instrument||'piano',a.target.reference||'none',a.target.key||'C major',a.target.meter||'4/4']);
...
export function validateScore(s){
 if(!s||!METERS.includes(s.meter)||!KEYS[s.key||'C major']||!Number.isInteger(s.bars)||s.bars<1||s.bars>4||!Number.isInteger(s.level)||s.level<0||s.level>6||!Number.isFinite(s.bpm)||s.bpm<40||s.bpm>160||!Array.isArray(s.events)||s.events.length>128)throw Error('Invalid score in backup.');
