...
export function schedule(score,support='countin'){
 const beat=60/score.bpm,reference=score.reference||'none',referenceNotes=reference==='chord'?[60,64,67]:reference==='pitch'?[60]:reference==='first'?[midi(score.events.find(e=>!e.rest&&e.step!==null)||{rest:true})??60]:[],referenceLength=referenceNotes.length?1.8:0;
 const lead=referenceLength+(support==='none'?0:4*beat),notes=sounding(score.events).filter(e=>!e.rest&&midi(e)!==null&&e.duration>0).map(e=>({time:lead+e.onset/Q*beat,duration:e.duration/Q*beat,midi:midi(e)})),clicks=[];
 if(support!=='none')for(let i=0;i<4;i++)clicks.push({time:referenceLength+i*beat,accent:i===0});
 ...
 return {notes,referenceNotes:referenceNotes.map(midi=>({midi,time:0,duration:1.2,gain:referenceNotes.length===3?.5:.8})),clicks,length:lead+score.bars*4*beat+.15};
}
