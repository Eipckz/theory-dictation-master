import {name,Q,barTicks,beatTicks,beatCount,meterInfo,beatIndex} from './domain.js';
...
export function staffSVG(score,{selected=-1,grid=false,readonly=false,title='Your transcription'}={}){
 const events=score.events,total=Math.max(score.bars*barTicks(score),events.reduce((s,e)=>s+(e.duration??Q),0)),shortest=Math.min(Q,...events.filter(e=>e.duration).map(e=>e.duration)),width=Math.min(16000,Math.max(640,total/shortest*44+150)),xpos=t=>120+t/total*(width-158),pos=events.map(e=>[xpos(e.onset)+12,yFor(e.step??34)]);
 ...
 if(grid){const m=meterInfo(score);for(let bar=0;bar<score.bars;bar++)m.pulses.forEach((t,i)=>{svg+=`<path d="M${xpos(bar*m.span+t)} 98V211" class="grid-line"/><text x="${xpos(bar*m.span+t)+10}" y="225" class="beat-label">${i+1}</text>`;});}
 const groups=[];let group=[];events.forEach((e,i)=>{if(!e.rest&&e.step!==null&&e.duration&&e.duration<Q){if(group.length&&beatIndex(score,events[group[0]].onset)!==beatIndex(score,e.onset)){if(group.length>1)groups.push(group);group=[];}group.push(i);}else{if(group.length>1)groups.push(group);group=[];}});if(group.length>1)groups.push(group);const beamed=new Set(groups.flat());let accidentals={},bar=-1;
 ...
 if(e.rest){const code=({48:'E4E3',24:'E4E4',12:'E4E5',6:'E4E6',3:'E4E7'})[base]??'E4E5';svg+=`<text x="${x-6}" y="162" class="music rest">&#x${code};</text>`;}
  ...
  const nextBar=Math.floor(e.onset/barTicks(score));if(nextBar!==bar){bar=nextBar;accidentals={};}if(e.step!==null&&(accidentals[e.step]??0)!==e.acc)svg+=`<text x="${x-25}" y="${y+7}" class="accidental">${({'-2':'♭♭','-1':'♭',0:'♮',1:'♯',2:'𝄪'})[e.acc]}</text>`;accidentals[e.step]=e.acc;
 }
