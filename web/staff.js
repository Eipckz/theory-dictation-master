...
export function staffSVG(score,{selected=-1,grid=false,readonly=false,title='Your transcription'}={}){
 const events=score.events,total=Math.max(score.bars*48,events.reduce((s,e)=>s+(e.duration??Q),0)),shortest=Math.min(Q,...events.filter(e=>e.duration).map(e=>e.duration)),width=Math.max(640,total/shortest*44+150),xpos=t=>120+t/total*(width-158),pos=events.map(e=>[xpos(e.onset)+12,yFor(e.step??34)]);
 let svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} 300" width="${width}" height="300" role="img" aria-label="${esc(title)}: ${events.length} entered events" ${readonly?'':'tabindex="0"'} class="score-svg"><rect width="100%" height="100%" fill="#F6F1E5"/><text x="24" y="27" class="score-label">${esc(title.toUpperCase())}</text><text x="24" y="49" class="score-context">C major · Treble · 4/4 · ${score.bars} bar${score.bars===1?'':'s'}</text>`;
 for(let i=0;i<5;i++)svg+=`<path d="M24 ${184-i*14}H${width-24}" class="staff-line"/>`;
 svg+=`<text x="30" y="170" class="music clef">&#xE050;</text><text x="82" y="151" class="meter">4</text><text x="82" y="178" class="meter">4</text>`;
 for(let b=0;b<=score.bars;b++)svg+=`<path d="M${xpos(b*48)} 128V184" class="staff-line"/>`;
 if(grid)for(let b=0;b<score.bars*4;b++)svg+=`<path d="M${xpos(b*Q)} 98V211" class="grid-line"/><text x="${xpos(b*Q)+10}" y="225" class="beat-label">${b%4+1}</text>`;
 const groups=[];let group=[];events.forEach((e,i)=>{if(!e.rest&&e.step!==null&&e.duration&&e.duration<Q){if(group.length&&Math.floor(events[group[0]].onset/Q)!==Math.floor(e.onset/Q)){if(group.length>1)groups.push(group);group=[];}group.push(i);}else{if(group.length>1)groups.push(group);group=[];}});if(group.length>1)groups.push(group);const beamed=new Set(groups.flat());let accidentals={},bar=-1;
 ...
 if(e.rest){const code=({48:'E4E3',24:'E4E4',12:'E4E5',6:'E4E6',3:'E4E7'})[base]??'E4E5';svg+=`<text x="${x-6}" y="162" class="music rest">&#x${code};</text>`;}
  ...
  const nextBar=Math.floor(e.onset/48);if(nextBar!==bar){bar=nextBar;accidentals={};}if(e.step!==null&&(accidentals[e.step]??0)!==e.acc)svg+=`<text x="${x-25}" y="${y+7}" class="accidental">${({'-1':'♭',0:'♮',1:'♯'})[e.acc]}</text>`;accidentals[e.step]=e.acc;
 }
